...
import { firestore, fs } from '../../../../fb-configs'
...
const AccountDetailsScreen = () => {
    ...
    const addUserDetails = async () => {
        ...
        // fs.collection('users').doc(uid).set({ ...values, _id: uid })
        ...
        <>
            <Screen style={CONTAINER} preset="scroll" >
                ...
                <View style={[styles.inputWrapper, INPUTVIEW]}>
                    {TextInputField(values.fullName, handleChange('fullName'), 'Name', nameRef, handleBlur('fullName'))}
                    {TextInputField(values.gender, handleChange('gender'), 'Gender', genderRef, handleBlur('gender'))}
                    {TextInputField(values.bloodType, handleChange('bloodType'), 'Blood Type', bloodTypeRef, handleBlur('bloodType'))}
                    {TextInputField(values.birthDate, handleChange('birthDate'), 'Date of Birth', bdRef, handleBlur('birthDate'))}
                    {TextInputField(values.insurance, handleChange('insurance'), 'Insurance', insuranceRef, handleBlur('insurance'))}
                    <Text style={[fontStyles.subHeadBold, { marginVertical: scaleByDeviceWidth(16) }]} textColor={color.palette.black}>{'Contact'}</Text>
                    {TextInputField(values.mobileNumber, handleChange('mobile'), 'Mobile Number', mobileRef, handleBlur('mobile'), false)}
                    {TextInputField(values.email, handleChange('email'), 'Email', emailRef, handleBlur('email'), false)}
                    <Text style={[fontStyles.subHeadBold, { marginVertical: scaleByDeviceWidth(16) }]} textColor={color.palette.black}>{'Address'}</Text>
                    {TextInputField(values.country, handleChange('country'), 'Country', countryRef, handleBlur('country'))}
                    {TextInputField(values.city, handleChange('city'), 'City', cityRef, handleBlur('city'))}
                    {TextInputField(values.address, handleChange('address'), 'Address', addressRef, handleBlur('address'))}
                </View>
                <Button onPress={handleSubmitForm} text={t('common.confirm')} textStyle={fontStyles.bodyRegular} style={{ marginTop: scaleByDeviceWidth(56), marginBottom: scaleByDeviceWidth(keyboardOpen ? 16 : 56) }}></Button>
            </Screen>
        </>
    )
}
